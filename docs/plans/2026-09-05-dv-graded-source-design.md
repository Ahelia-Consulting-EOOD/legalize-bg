...
- MCP and REST: `get_law` and the search hit carry `provenance_grade`, `checked_through` and `chain_inherited_before`; a single warning class `PROVENANCE_GRADE` carrying the grade in its payload rides in every successful response for any grade other than A, B-pending included (D-064), the way `IMPLICIT_ALINEA` does (D-058), so a consumer never receives snapshot text unlabelled; the disclaimer and tie-break rule travel in `tools.json` response metadata so that dropping them is a visible contract violation (BOE obliges reusers to carry its label and update date; section 12).
...
## 11. Decisions on the open questions (owner, 2026-09-05; recorded as D-064)

The owner answered the six questions in the working tree on 2026-09-05, deciding 1, 2 and 6 directly and delegating 3, 4 and 5 („you have better data to take the best decision, do it“). The delegated decisions are taken here with their reasoning and are the owner's to overturn.

1. **Order of the grade A batch after the pilot: by consumer priority.** The DRS and takt-plan act lists first, then the remaining single-issue acts by recency. (Owner.)
2. **PDF-era reading order for grade B: by act importance.** The same consumer-priority lists first, then the civil-law backbone (ЗЗД, ЗС, ЗН, ЗЛС, НК, КТ), then the rest by event count. (Owner.)
3. **Warning shape on the wire: one warning class, `PROVENANCE_GRADE`, carrying the grade in its payload** (`grade`, `pending_items`, `checked_through`, `chain_inherited_before`), emitted in every successful response for any grade other than A, B-pending included. Reasoning: D-058's `IMPLICIT_ALINEA` set the precedent of one class per signal; the error taxonomy (`docs/api/error-codes.json`, `tools.json`) is versioned and parity-tested, so one additive entry costs one Surface 3 preflight while a class per grade would cost four and give consumers nothing a payload field does not; a consumer filters on one code and reads the grade. (Delegated.)
4. **Identifier form for ДВ-only acts: `identificador = dv-<idMat>` of the promulgating Gazette material.** `fuente = dv.parliament.bg`; `eli` is built by the existing rule from rango, Gazette date and slug, which never depended on lex.bg; `Norm-Id` in commits is the same identifier; the synthetic identifier the D-049 addendum gave Указ 883 (`-883197400`) migrates to this form when its material is located, or is waived. Reasoning: the `dv-` prefix cannot collide with lex.bg's numeric ids; `idMat` is unique and immutable on the Gazette side and its URL resolves to the official text, which is what an identifier should point at. Executed through the Surface 2 preflight (frontmatter) and named in the Surface 5 preflight (commit form). (Delegated.)
5. **Chain-omission and `estado` findings before the engine: recorded as data, never committed as events, until the single write gate exists.** After the write gate lands, a repeal confirmed by the reasoning pass may flip `estado` to `derogado` through a `[otmyana]` commit made by the pipeline, in one batch (the 2026-06-21 evaluation set that precedent with 17 such commits, and P0-6 made re-scrape respect them). `[reforma]` events wait for the engine: a `[reforma]` commit without an applied text change would mint a false version boundary in the FR-020 time machine. Until then the findings live in `chain-omissions.csv` and `estado-disputes.csv` and, once the provenance block ships, as the `estado_disputed` uncertainty flag on the act. (Delegated.)
6. **PDF-era tables of contents: not read now; an inventory is prepared for the owner's token-cost evaluation.** The coverage map emits `pdf-era-inventory.csv`, one row per issue from 1989 to бр. 42/2005: year, number, date, `idObj`, the number of corpus events that cite the issue, the estimated table-of-contents pages, the estimated pages of the corpus-referenced materials, and the estimated total pages of the issue; with totals for the table-of-contents pages, the corpus-referenced pages and the overall page count. Page model: table-of-contents pages = the first material's start page minus one, and material lengths from consecutive start pages, both measured over the HTML-era issues and applied to the PDF era by act type and decade; every figure is labelled an estimate until an issue PDF is opened. (Owner.)
