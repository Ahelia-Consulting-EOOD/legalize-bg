...
| 3 | any event `applied = pending` (this includes every `unlocated` event), or `chain_scan_complete` false, or `base.state = snapshot` with the promulgation online and `base.audited` false, or `base.state = snapshot` with `base.frozen_at` null | **B-pending**, with the pending items enumerated in the record: unread or unverified events, the missing chain scan, the missing base audit, the freeze not yet run |
| 4 | everything else: a snapshot or read base, frozen and audited, every event in scope `replayed`, `verified` or `not_incorporated`, chain scan complete | **B, ДВ-audited** |

The procedure is total: rule 1 catches every offline history, rule 2 the Gazette-complete ones, rule 3 every open item, and rule 4 the rest. Case checks from the review: two HTML events verified before the sweep has run is B-pending (rule 3, freeze null), which is the honest state for every act audited before owner decision O-3; a `not_incorporated` event blocks nothing and warns everywhere; a 2003 act whose base is a PDF-era promulgation is B-pending until that base has been read (state `read`) or, if the owner declares 2005 as its base date, B on its post-2005 events with the declaration surfaced; an HTML-era event the resolver cannot attribute is `unlocated` and `pending`, so it blocks B; a rebuilt act with open divergences is never a committed file.
...
Outputs: `coverage-map.csv` (one row per event), `acts-summary.csv` (one row per act with candidate grade, pending items, page estimate), `chain-omissions.csv`, `unresolved.csv` (events and instructions the resolver could not attribute, and the seven corpus acts with an empty `titulo`, which cannot be resolved by title at all), and a short report with the totals by grade and by decade. The map is regenerated by the pipeline whenever the tables change; it is a research artifact until the provenance block ships, then it becomes the derivation input for the grade.
