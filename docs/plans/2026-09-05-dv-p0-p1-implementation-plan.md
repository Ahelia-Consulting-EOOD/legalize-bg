...
**Inputs beyond `unresolved.csv`.** Rows of `coverage-map.csv` whose `resolver_flags` contain `content_mismatch` or `numbered_key_tie` are reviewed in the reasoning pass alongside `unresolved.csv`, because they are attributions the resolver made across a changed content word or a non-unique key.
