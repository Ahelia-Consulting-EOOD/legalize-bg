...
> **🆕 2026-07-01 (late) — BULK RE-SCRAPE PENDING (D-047 remediation, next execution step).** The CF-clearance fetch path shipped (D-047 Task 9); **396 laws re-scraped on `origin/refresh/2026-06-29-parser-fix` (`cbd1bbd4`, 0 gate-fail)**; DRS-priority acts restored on `origin/main` (`c3f0c49a`). **Remaining: the ~3,203 non-law acts** (`--categories code,ords,regs,reg_laws`). **Committer-date discipline changed (D-048):** corpus commits no longer backdate `GIT_COMMITTER_DATE`. **New P1 defect (D-049 / FR-025):** the corpus omits whole lex.bg act-type categories (ПМС etc.) + приложение-as-separate-doc — **next due AFTER the bulk**. **▶ Bulk procedure: `docs/sync/HANDOFFS/2026-07-01-bulk-rescrape-handover.md`; cookie mint: `docs/runbook/2026-07-01-cf-cookie-mint.md`.**
