...
- **MISSING acts — evaluated + actioned** (`docs/sync/HANDOFFS/2026-06-21-missing-acts-evaluation.md`): 17 confirmed repeals flipped to `estado: derogado` via 17 `[otmyana]` commits, each author-dated at the real repeal date; index rebuilt (17 `derogado`). **1 outlier pending owner decision:** `2137255124` (Union of Architects private bylaw, no ДВ promulgation) was correctly NOT flipped — choose remove-as-erroneous vs keep-but-mark.
- **Open items for the owner:** (1) decide the #18 outlier above. (2) 16 acts classified as changed but re-assembled byte-identical (idempotent guard absorbed them, 0 empty commits) — benign report over-count; root cause is an amendment_history set-vs-serialization edge in `history_grew`, a candidate FR only if it recurs at scale.
