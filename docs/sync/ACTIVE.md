...
**Next action:** **FR-025 / D-049 — the corpus omits whole lex.bg act-type categories** (ПМС confirmed absent via НСС; likely тарифи/инструкции/решения/укази) + приложение-as-separate-doc. Enumerate ALL lex.bg categories → owner scope decision → add corpus dirs + discovery + index + `rango` (protected surfaces → IMPLEMENTATION-PREFLIGHT) → a corpus-level coverage gate (set-level analogue of the D-047 per-act gate) → fetch via the CF-hardened pipeline (D-047 Task 9 path A). **THEN** resume the national-functionality track: Phase 3 (DV monitor / freshness) → deterministic consolidation engine (Phase 4, LawVM-style, oracle-validated) → FR-024 forward re-source → FR-009 full historical re-source (optional) → FR-022 municipal (last). Roadmap: `docs/sync/HANDOFFS/2026-06-22-freshness-consolidation-resource-roadmap.md`.
