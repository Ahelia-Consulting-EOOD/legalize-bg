...
CREATE TABLE provisions (
    ...
    implicit INTEGER NOT NULL DEFAULT 0  -- added by Migration 006
);
```
...
| `implicit` | INTEGER | NOT NULL, default 0 (0/1 boolean) | FR-034 marker: 1 when the row's `paragraph` number was DERIVED from the paragraph's POSITION inside the article rather than read from an `(N)` marker — acts promulgated before Указ № 883/1974 print their алинеи unnumbered. Always 0 for the article-as-whole row (`paragraph IS NULL`) and for alineas carrying an explicit marker. Written by `index/build.py` from `index/provisions.py:Provision.implicit`; read by `mcp_server/queries.py:article_lookup` and surfaced as `get_article`'s `implicit` field plus an `IMPLICIT_ALINEA` warning. Added by Migration 006. |
...
The base schema in §2 (laws / law_versions / amendments / provisions + the three indexes) is the Phase 1a shape. Phase 1b.1 evolved the schema via four forward-only migrations; FR-032 added migration 005 (schema version 5, the per-segment FTS split); FR-034 added migration 006 (schema version 6, the implicit-alinea marker); future phases will add more.
...
| 6 | `provisions_implicit_column` | `ALTER TABLE provisions ADD COLUMN implicit INTEGER NOT NULL DEFAULT 0;` | FR-034 — marks alinea rows whose number was derived from paragraph POSITION (pre-Указ-883/1974 acts print unnumbered алинеи). NOT NULL DEFAULT 0 keeps the change strictly additive: every row indexed before the migration, and every INSERT that omits the column, reads back as 0. Purely additive on the wire — `get_article` gains an `implicit` boolean and an `IMPLICIT_ALINEA` warning (tools.json 1.5.0). A rebuild is required only to POPULATE the flag; the migrated catalog stays correct (all-zero) until then. |
